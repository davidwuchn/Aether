...
func surveyDispatchContract() map[string]interface{} {
	return codexDispatchContract{
		...
		DependencyBehavior:   "Surveyors are independent read-only workers; real dispatch requires the codex CLI in PATH.",
...
func planningDispatchContract() map[string]interface{} {
	return codexDispatchContract{
		...
		DependencyBehavior:   "Real worker dispatch requires the codex CLI in PATH. Route-setter execution depends on the scout completing first.",
